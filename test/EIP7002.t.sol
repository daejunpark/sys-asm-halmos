 ...
 *
 * This test verifies that the geas implementation conforms to the specifications in EIP-7002.
 * It ensures the correctness of withdrawal request processing, fee calculations, and storage integrity.
...
contract EIP7002Test is SymTest, Test {
    ...
    // auxiliary constants
    uint256 constant SLOTS_PER_ITEM = 3;   // padding (12 bytes) + address (20 bytes) + pubkey (48 bytes) + amount (8 bytes) + padding (8 bytes)
    uint256 constant INPUT_SIZE = 56;      //                                           pubkey (48 bytes) + amount (8 bytes)
    uint256 constant RECORD_SIZE = 76;     //                      address (20 bytes) + pubkey (48 bytes) + amount (8 bytes)
    ...
    function setUp() public {
        ...
        vm.etch(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, Geas.compile("sys-asm/src/withdrawals/main.eas"));
    ...
    /// @custom:halmos --array-lengths data={0,1,2,32,56,64,1024}
    function check_invariant(address caller, uint256 value, bytes memory data) public {
        ...
        assertLe(newState.queueHeadIndex, newState.queueTailIndex, "invariant: head <= tail");
    }
    ...
    function check_user_operation(address caller, uint256 value, bytes memory data) public {
        ...
        if (data.length == 0) {
            if (success) {
                assertEq(uint256(bytes32(retdata)), _getFee(), "get: retdata");
                // ensure no storage updates
                assertEq(newState.anySlotValue, initState.anySlotValue, "get: storage");
                // ensure zero callvalue
                // introduced in: https://github.com/lightclient/sys-asm/pull/28
                assertEq(value, 0, "get: zero callvalue");
            } else {
                // NOTE: the contract immediately reverts when excess == EXCESS_INHIBITOR
                if (initState.excess != EXCESS_INHIBITOR) {
                    // ensure that the failure is only due to non-zero callvalue
                    assertNotEq(value, 0, "get: non-zero callvalue");
                }
            }
        ...
        // add_withdrawal_request() operation
            ...
            if (success) {
                ...
                assertEq(newState.count, initState.count + 1, "add: count");

                // ensure new queue element
                assertEq(newState.queueTailIndex, initState.queueTailIndex + 1, "add: tail index");
                ...
                assertEq(queueItem, bytes.concat(bytes32(uint256(uint160(caller))), data, bytes8(0)), "add: queue item");
                // ensure no dirty bits for source address, and zero padding at the end
                assertEq(bytes12(this.slice(queueItem, 0, 12)), bytes12(0), "add: left zero padding");
                assertEq(bytes8(this.slice(queueItem, 88, 96)), bytes8(0), "add: right zero padding");

                // ensure sufficient fee
                assertGe(value, _getFee(), "add: sufficient callvalue");
                ...
                if (
                    ...
                    assertEq(newState.anySlotValue, initState.anySlotValue, "add: storage");
                }

                // ensure empty return data
                assertEq(retdata.length, 0, "add: retdata");
            } else {
                // NOTE: the contract immediately reverts when excess == EXCESS_INHIBITOR
                if (initState.excess != EXCESS_INHIBITOR) {
                    // ensure that the failure is only due to insufficient fee
                    assertLt(value, _getFee(), "add: insufficient callvalue");
                }
            }
        ...
        // invalid user operation
            ...
            assertFalse(success, "invalid user operation");
        }
    }

    /// @custom:halmos --array-lengths data={1024}
    function check_system_operation(address caller, uint256 value, bytes memory data) public {
        ...
        assertTrue(success, "system: success");

        // ensure excess update
        if (initState.excess != EXCESS_INHIBITOR) {
            assertEq(newState.excess, subcap(initState.excess + initState.count, TARGET_WITHDRAWAL_REQUESTS_PER_BLOCK), "system: excess");
        } else {
            // at the fork block
            assertEq(newState.excess, subcap(                   initState.count, TARGET_WITHDRAWAL_REQUESTS_PER_BLOCK), "system: excess at fork");
        }

        // ensure count reset
        assertEq(newState.count, 0, "system: count");
        ...
        if (oldQueueSize <= MAX_WITHDRAWAL_REQUESTS_PER_BLOCK) {
            assertEq(newState.queueHeadIndex, 0, "system: reset head index");
            assertEq(newState.queueTailIndex, 0, "system: reset tail index");
        } else {
            assertEq(newState.queueTailIndex, initState.queueTailIndex, "system: tail index");
        }

        // ensure max withdrawals per block
        assertLe(numDequeued, MAX_WITHDRAWAL_REQUESTS_PER_BLOCK, "system: # of dequeued");

        // ensure retdata size
        assertEq(retdata.length, RECORD_SIZE * numDequeued, "system: retdata size");
        ...
        for (uint256 i = 0; i < numDequeued; i++) {
            ...
            if (svm.createBool("check-retdata")) {
                ...
                assertEq(_getSource(queueCurrItem), address(uint160(bytes20(this.slice(retdata, retOffset + 0, retOffset + 20)))), "system: retdata source");

                // check validator pubkey
                assertEq(_getPubkey(queueCurrItem), this.slice(retdata, retOffset + 20, retOffset + 68), "system: retdata pubkey");

                // ensure big-endian amount
                assertEq(_getAmountBE(queueCurrItem), bytes8(this.slice(retdata, retOffset + 68, retOffset + RECORD_SIZE)), "system: retdata amount");
        ...
        if (anySlot >= WITHDRAWAL_REQUEST_QUEUE_STORAGE_OFFSET) {
            assertEq(newState.anySlotValue, initState.anySlotValue, "system: storage");
        }
    }
    ...
    function _check_balance_update(State memory newState, uint256 value, address caller, uint256 callerBalance) internal view {
        if (caller != WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS) {
            ...
            assertEq(caller.balance, callerBalance, "balance: caller alias");
            // new balance set earlier by vm.deal(caller, callerBalance)
            assertEq(newState.balance, callerBalance, "balance: contract alias");
        }
    }
    ...
    function _getState() internal view returns (State memory) {
        uint256 balance = WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS.balance;

        uint256 excess = uint256(vm.load(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, bytes32(EXCESS_WITHDRAWAL_REQUESTS_STORAGE_SLOT)));
        uint256 count = uint256(vm.load(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, bytes32(WITHDRAWAL_REQUEST_COUNT_STORAGE_SLOT)));

        uint256 queueHeadIndex = uint256(vm.load(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, bytes32(WITHDRAWAL_REQUEST_QUEUE_HEAD_STORAGE_SLOT)));
        uint256 queueTailIndex = uint256(vm.load(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, bytes32(WITHDRAWAL_REQUEST_QUEUE_TAIL_STORAGE_SLOT)));

        uint256 anySlotValue = uint256(vm.load(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, bytes32(anySlot)));
