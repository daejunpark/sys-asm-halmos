...
contract EIP7251Test is SymTest, Test {
    ...
    function setUp() public {
        ...
        vm.etch(CONSOLIDATION_REQUEST_PREDEPLOY_ADDRESS, Geas.compile("sys-asm/src/consolidations/main.eas"));
    ...
    /// @custom:halmos --array-lengths data={0,1,2,32,64,96,1024}
    ...
    function check_user_operation(address caller, uint256 value, bytes memory data) public {
        ...
        // add_consolidation_request() operation
            ...
            if (success) {
                ...
                assertEq(queueItem, bytes.concat(bytes32(uint256(uint160(caller))), data), "add: queue item");
                // ensure no dirty bits for source address
                assertEq(bytes12(this.slice(queueItem, 0, 12)), bytes12(0), "add: zero padding");
    ...
    function check_system_operation(address caller, uint256 value, bytes memory data) public {
        ...
        if (initState.excess != EXCESS_INHIBITOR) {
            assertEq(newState.excess, subcap(initState.excess + initState.count, TARGET_CONSOLIDATION_REQUESTS_PER_BLOCK), "system: excess");
        } else {
            // at the fork block
            assertEq(newState.excess, subcap(                   initState.count, TARGET_CONSOLIDATION_REQUESTS_PER_BLOCK), "system: excess at fork");
        }
        ...
        assertLe(numDequeued, MAX_CONSOLIDATION_REQUESTS_PER_BLOCK, "system: # of dequeued");
        ...
        for (uint256 i = 0; i < numDequeued; i++) {
            ...
            if (svm.createBool("check-retdata")) {
                ...
                // check source pubkey
                assertEq(_getSourcePubkey(queueCurrItem), this.slice(retdata, retOffset + 20, retOffset + 68), "system: retdata source pubkey");

                // check target pubkey
                assertEq(_getTargetPubkey(queueCurrItem), this.slice(retdata, retOffset + 68, retOffset + RECORD_SIZE), "system: retdata target pubkey");
